...
export function setupGithub({ server }) {
  ...
  const CLIENT_ID = dev
    ? process.env.Github_Test_ClientID
    : process.env.Github_Live_ClientID;
  const API_KEY = dev
    ? process.env.Github_Test_SecretKey
    : process.env.Github_Live_SecretKey;

  server.get("/auth/github", (req, res) => {
    if (!req.user || !req.user.isAdmin) {
      res.redirect("/login");
    } else {
      const oauthEndpoint = `${AUTHORIZE_URI}?${qs.stringify({
        scope: "repo",
